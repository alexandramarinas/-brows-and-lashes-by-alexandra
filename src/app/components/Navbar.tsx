...
export default function Navbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition ${scrolled || open ? "border-b border-stone-200 bg-white/95 backdrop-blur" : "bg-transparent"}`}>
      ...
      {open && <nav className="border-t border-stone-200 bg-white px-6 py-4 md:hidden">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="block py-3 text-sm">{label}</a>)}<a href="https://wa.me/40722808515" target="_blank" rel="noreferrer" className="mt-2 block bg-[#745644] px-4 py-3 text-center text-xs uppercase tracking-wider text-white">Programează-te</a></nav>}
    </header>
  );
}
